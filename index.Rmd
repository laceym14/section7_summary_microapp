...
agencies <- c("*", levels(full$lead_agency))
names(agencies) <- c("All", levels(full$lead_agency))

work_cats <- c("*", levels(full$work_category))
names(work_cats) <- c("All", levels(full$work_category))
...
species <- c("*", as.character(levels(as.factor(unlist(unlist(full$spp_ev_ls))))))
names(species)<- c("All", as.character(levels(as.factor(unlist(unlist(full$spp_ev_ls))))))
...
#fluidPage(
  fluidRow(
    column(9, leafletOutput("states_map", height="200px", width = "100%")),
    column(3, 
             br(),
